...
function sendToCraft(markdown, article) {
  const search = new URLSearchParams({
    spaceId: "c15897ad-7afe-d68a-0509-3604aed57093",
  });
  const title = encodeURIComponent(article.title);
  const content = encodeURIComponent(markdown);
  const url = `craftdocs://createdocument?${search}&title=${title}&content=${content}`;
  if (chrome) {
